...
<html>
	<head>
		...
		<title>Admin - Create Spbu</title>
	</head>
	<body>
		<a href="index.htm">Spbu List</a>
		<sf:errors path="*"/>
		<sf:form commandName="formData" data-ajax="false" action="add.htm" method="post">
			<fieldset>
				...
				<section>
					...
					<div>
						<input type="number" name="machineCount" id="machineCount" min="0" class="g1 integer" value="${formData.machineSerials.size()}"/><br/>
						<table id="tblMachines">
							...
							<tbody>
								<c:forEach items="${formData.machineSerials}" var="machineSerial" varStatus="machineSerialStatus">
									<tr>
										<td><input name="machineSerials" type="text" value="${machineSerial}" id="machineSerial_${machineSerialStatus.index}"/></td>
										<td><input name="modelIds" type="text" value="${formData.modelIds[machineSerialStatus.index]}" id="modelId_${machineSerialStatus.index}"/></td>
										<td><input name="machineIdentifiers" type="text" value="${formData.machineIdentifiers[machineSerialStatus.index]}" id="machineIdentifier_${machineSerialStatus.index}"/></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
				...
				$("#machineCount").change(function() {
					...
					if (isFinite(machineCountStr)) {
						...
						for (var i = currentMachineCount; i < machineCount; i++) {
							...
							newRow += '<tr>';
							newRow += '<td>' + '<input name="machineSerials" type="text" value="" id="' + machineSerialId + '"/>' + '</td>';
							newRow += '<td>' + '<input name="modelIds" type="text" value="" id="' + modelIdId + '"/>' + '</td>';
							newRow += '<td>' + '<input name="machineIdentifiers" type="text" value="" id="' + machineIdentifierId + '"/>' + '</td>';
							newRow += '</tr>';
