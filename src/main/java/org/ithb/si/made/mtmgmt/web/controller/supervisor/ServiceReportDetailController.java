...
import org.ithb.si.made.mtmgmt.web.controller.technician.*;
 ...
 * @author gde
 */
@Controller("supervisorServiceReportDetailController")
@RequestMapping("/supervisor/service_report_detail")
public class ServiceReportDetailController {
	...
	public String showFailureHistoryDetail(Principal principal, Model model, @RequestParam("serviceReportId") long serviceReportId) {
		...
		final UserEntity supervisorEntity = userRepository.findByLoginId(principal.getName());
		final ServiceReportEntity serviceReport = serviceReportRepository.findOne(serviceReportId);
		if (serviceReport == null) {
			return "common/hacker";
		}
		if (!serviceReport.getSpbuMachineEntity().getSpbuEntity().getSupervisorEntity().getId().equals(supervisorEntity.getId())) {
			return "common/hacker";
		}
